"""Playwright driver for Phase 6 — full pipeline from raw counts in browser."""
...
def main():
    ...
    url = f"http://127.0.0.1:{args.port}/web/phase6.html"
    ...
    with sync_playwright() as p:
        ...
        for backend in args.backends:
            ...
            try:
                page.evaluate(f"runPhase6('{backend}')")
                r = page.evaluate("window.phase6Result")
                results.append(r)
                print(f"  weights_diff: {r['weights_max_diff']:.2e}  "
                      f"min_cosine: {r['min_cosine']:.6f}  "
                      f"mean_cosine: {r['mean_cosine']:.6f}  "
                      f"hist_pearson: {r['hist_pearson']:.4f}  "
                      f"mean_ms: {r['mean_ms']:.1f}")
                # Normalize is deterministic (expect weights diff ~ float32 eps).
                # Sampler noise floor matches Phase 5.
                ok = (
                    r["weights_max_diff"] < 1e-5
                    and r["min_cosine"] > 0.88
    ...
    for r in results:
        if "error" in r:
            raise SystemExit(1)
        if (
            r.get("weights_max_diff", 1) >= 1e-5
            or r.get("min_cosine", 0) <= 0.88

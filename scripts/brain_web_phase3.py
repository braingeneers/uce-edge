"""Playwright driver for Phase 3 — gather + transformer in the browser."""
...
def main():
    ...
    ap.add_argument("--timeout", type=int, default=300_000,
                    help="Per-call timeout in ms (default 300s to cover first-load table fetch)")
    args = ap.parse_args()

    os.chdir(REPO_ROOT)
    httpd = start_server(args.port, str(REPO_ROOT))
    url = f"http://127.0.0.1:{args.port}/web/phase3.html"
    ...
    with sync_playwright() as p:
        ...
        for backend in args.backends:
            ...
            try:
                page.evaluate(f"runPhase3('{backend}')")
                r = page.evaluate("window.phase3Result")
                results.append(r)
                print(f"  gather_max_diff: {r['gather_max_diff']:.2e}  "
                      f"min_cosine: {r['min_cosine']:.6f}  "
                      f"mean_ms: {r['mean_ms']:.1f}")
                ok = r["gather_max_diff"] == 0 and r["min_cosine"] > 0.999
    ...
    for r in results:
        if "error" in r:
            raise SystemExit(1)
        if r.get("gather_max_diff", 1) != 0 or r.get("min_cosine", 0) <= 0.999:
            raise SystemExit(1)
